...
export default function ContentManagerTab() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const items = snap.docs.map(doc => doc.data() as YouTubeChannel);
      setChannels(items);
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const items = snap.docs.map(doc => {
        ...
        return {
          id: doc.id,
          playlistId: d.playlistId || doc.id,
          title: d.title,
          description: d.description || '',
          thumbnailUrl: d.thumbnailUrl || d.thumbnail || '',
          channelId: d.channelId,
          channelName: d.channelName,
          videoCount: d.videoCount ?? d.lecturesCount ?? 0,
          lecturesCount: d.lecturesCount ?? d.videoCount ?? 0,
          subject: d.subject || '',
          examType: d.examType || 'NEET',
          examTags: d.examTags || d.examTypes || [],
          isActive: d.isActive !== false,
          importStatus: d.importStatus || 'pending'
        } as Playlist;
      });
      setPlaylists(items);
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'videos'), orderBy('publishedAt', 'desc'), limit(500));
    const unsub = onSnapshot(q, (snap) => {
      const items = snap.docs.map(doc => {
        const d = doc.data();
        return {
          ...
          title: d.title,
          description: d.description || '',
          thumbnail: d.thumbnail || d.thumbnailUrl || '',
          duration: d.duration || '',
          durationSeconds: d.durationSeconds || 0,
          publishedAt: d.publishedAt || d.publishDate || d.importedAt || new Date().toISOString(),
          viewCount: d.viewCount || d.viewsCount || 0,
          likeCount: d.likeCount || d.likesCount || 0,
          subject: d.subject || '',
          topic: d.topic || '',
          examTags: d.examTags || [],
          isActive: d.isActive !== false,
  ...
  useEffect(() => {
    ...
    const q = query(collection(db, 'syncLogs'), orderBy('timestamp', 'desc'), limit(150));
    const unsub = onSnapshot(q, (snap) => {
      const items = snap.docs.map(doc => doc.data() as YouTubeSyncLog);
      setSyncLogs(items);
      setIsLoadingLogs(false);
    }, (error) => {
      console.error('Error listening to sync logs:', error);
      setIsLoadingLogs(false);
    });
    return () => unsub();
  }, []);

  // Real-time tracker for newly importing channel stats
  useEffect(() => {
    if (!activeImportId) return;

    pushTerminalLog(`Attaching real-time listener to Firestore sync logs and collections for ID: ${activeImportId}...`);

    // Listen to collection queries matching currently adding channel
    const playlistQ = query(collection(db, 'playlists'), where('channelId', '==', activeImportId));
    const videoQ = query(collection(db, 'videos'), where('channelId', '==', activeImportId));
    const channelDocRef = doc(db, 'channels', activeImportId);

    const unsubPlaylists = onSnapshot(playlistQ, (snap) => {
      const count = snap.size;
      setImportPlaylistsProgress(prev => ({ ...prev, current: count }));
      if (count > 0) {
        pushTerminalLog(`Live Ingestion: Playlists catalog updated. Imported Playlists: ${count}`);
      }
    });

    const unsubVideos = onSnapshot(videoQ, (snap) => {
      const count = snap.size;
      setImportVideosProgress(prev => ({ ...prev, current: count }));
      if (count > 0) {
        pushTerminalLog(`Live Ingestion: Lectures catalog updated. Imported Videos: ${count}`);
      }
    });

    const unsubChannel = onSnapshot(channelDocRef, (snapDoc) => {
      if (snapDoc.exists()) {
        const c = snapDoc.data() as YouTubeChannel;
        setImportFoundChannel(c);
        setImportPlaylistsProgress(prev => ({ ...prev, total: c.totalPlaylists || 20 }));
        setImportVideosProgress(prev => ({ ...prev, total: c.totalVideos || 500 }));
        pushTerminalLog(`Channel successfully resolved & saved: ${c.channelName} (${c.subscriberCount.toLocaleString()} pre-medical subscribers)`);
      }
    });

    // Listen to latest sync log triggered for this channel for error handling
    const logQ = query(collection(db, 'syncLogs'), where('targetId', '==', activeImportId), orderBy('timestamp', 'desc'), limit(1));
    const unsubLog = onSnapshot(logQ, (snapLogs) => {
      if (!snapLogs.empty) {
        const latestLog = snapLogs.docs[0].data() as YouTubeSyncLog;
        if (latestLog.status === 'failed') {
          setImportStatusStep('failed');
          pushTerminalLog(`❌ API sync log flagged failure: ${latestLog.error || 'Unknown quota exception'}`);
        }
      }
    });
    ...
    try {
      ...
      for (let idx = 0; idx < plList.length; idx++) {
        ...
        const itemRes = await fetch('/api/youtube/playlists/import', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ playlistId: playlist.playlistId || playlist.id })
        });
  ...
  const handleToggleChannelStatus = async (channelId: string, currentVal: boolean) => {
    try {
      await updateDoc(doc(db, 'channels', channelId), { isActive: !currentVal });
    } catch (err: any) {
  ...
  const handleOpenPlaylistVideos = async (pl: Playlist) => {
    ...
    try {
      ...
      const response = await fetch(`/api/youtube/lectures?playlistId=${pl.playlistId}`);
      if (response.ok) {
        const resData = await response.json();
        setPlaylistVideosList(resData.data || []);
      } else {
        // Fallback filter client-side videos
        const filtered = videos.filter(v => v.playlistId === pl.playlistId);
  ...
  const handleDeletePlaylistExecute = async (playlist: Playlist) => {
    ...
    try {
      ...
      // Batch delete associated videos to prevent orphan records
      const associatedVideos = videos.filter(v => v.playlistId === playlist.playlistId);
      if (associatedVideos.length > 0) {
        const batch = writeBatch(db);
        associatedVideos.forEach(v => {
  ...
  const handleTogglePlaylistActive = async (playlistId: string, currentVal: boolean) => {
    try {
      await updateDoc(doc(db, 'playlists', playlistId), { isActive: !currentVal });
    } catch (err: any) {
  ...
  const handleToggleVideoActive = async (videoId: string, currentVal: boolean) => {
    try {
      await updateDoc(doc(db, 'videos', videoId), { isActive: !currentVal });
    } catch (err: any) {
  ...
  const handleBulkHide = async (hide: boolean) => {
    ...
    try {
      ...
      selectedVideoIds.forEach(id => {
        ...
        batch.update(ref, { isActive: !hide });
      });
      await batch.commit();
      setSelectedVideoIds([]);
                  ...
                  {playlists
                    .filter(pl => {
                      ...
                      const matchExam = playlistsExamFilter === 'all' || 
                        (pl.examTags && pl.examTags.some(tag => tag.includes(playlistsExamFilter))) ||
                      ...
                      <tr key={pl.id} className="hover:bg-zinc-800/20 transition-all">
                        ...
                        <td className="py-3 px-4 whitespace-normal">
                          <div className="flex flex-wrap gap-1 max-w-[150px]">
                            {pl.examTags && pl.examTags.length > 0 ? (
                              pl.examTags.map(tag => (
                                <span key={tag} className="text-[9px] font-mono bg-zinc-800 text-zinc-300 px-1.5 py-0.5 rounded">
                                  {tag}
                                </span>
                              ))
                            ) : (
            ...
            <div className="md:col-span-3">
              ...
              >
                ...
                {playlists
                  ...
                  .map(p => (
                    <option key={p.id} value={p.playlistId}>{p.title}</option>
                  ))}
              </select>
            </div>
