  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-black flex flex-col items-center justify-center p-6 text-center space-y-6">
        ...
        <div className="w-16 h-16 rounded-full bg-zinc-90 w-fit p-4 bg-zinc-900 border border-[#222] flex items-center justify-center mx-auto text-zinc-400">
    ...
    <div className="w-full text-zinc-100 flex flex-col justify-start items-center relative py-6 px-4 md:px-8 select-none">
      ...
      <div className="w-full max-w-2xl mx-auto space-y-8 pt-2">
          ...
          <div className="flex items-center gap-2">
            {unreadCount > 0 && onMarkAllAsRead && (
              <button
                ...
                className="text-[10px] text-zinc-300 hover:text-white uppercase tracking-wider font-bold font-mono py-1.5 px-3.5 bg-[#0F0F10] hover:bg-zinc-900 border border-zinc-850 rounded-full cursor-pointer transition-all"
              >
                Mark All Read
              </button>
            )}
        ...
        {notifications.length === 0 ? (
          <div className="py-24 text-center space-y-4 bg-[#060607] border border-zinc-900 rounded-2xl">
            <div className="w-12 h-12 rounded-full bg-zinc-900/40 flex items-center justify-center mx-auto text-zinc-600">
              <Bell className="w-5 h-5 animate-pulse" />
            </div>
            <div className="space-y-1">
              <p className="text-xs font-semibold text-zinc-305">Your alert board is clear</p>
              <p className="text-[10.5px] text-zinc-500 font-mono">Swipe elements left or right to clear older alerts from this browser.</p>
            </div>
          </div>
        ) : (
              ...
              <AnimatePresence initial={false} mode="popLayout">
                {notifications.map((n) => (
                  <div key={n.id} className="relative overflow-visible rounded-xl">
                    ...
                    <div className="absolute inset-0 bg-rose-950/20 border border-rose-500/10 rounded-xl flex items-center justify-between px-6 text-rose-500/70 select-none pointer-events-none">
                    ...
                    <motion.div
                      ...
                      className={`p-4 rounded-xl border transition-all text-left relative flex gap-4 cursor-pointer active:cursor-grabbing select-none ${
                        n.read
                          ? 'border-zinc-900 bg-[#070708] text-zinc-550 hover:bg-[#0c0c0e] hover:border-zinc-850'
                          : 'border-white/15 bg-[#0D0D10] text-white hover:border-white/30 hover:bg-[#111115] shadow-[0_4px_24px_rgba(255,255,255,0.015)]'
                      }`}
                    >
                      {/* Left category indicator */}
                      <div className={`mt-0.5 w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                        n.read ? 'bg-zinc-905 text-zinc-600' : 'bg-white text-black font-extrabold'
                      }`}>
