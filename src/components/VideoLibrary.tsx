...
import { YouTubeChannel, YouTubeVideo, Playlist } from '../types';
...
export default function VideoLibrary({ onBackToHome }: VideoLibraryProps) {
  const { user } = useAuth();
  ...
  // Load Real-time Playlists & Channels on mount
  useEffect(() => {
    setIsLoadingFeed(true);
    
    // Listen to educational channels
    const channelsQuery = query(collection(db, 'channels'), where('isActive', '==', true));
    const unsubChannels = onSnapshot(channelsQuery, (snap) => {
      const items = snap.docs.map(doc => doc.data() as YouTubeChannel);
      setChannels(items);
    }, (err) => console.error("Error listening to channels:", err));

    // Listen to academic playlists
    const playlistsQuery = query(collection(db, 'playlists'));
    const unsubPlaylists = onSnapshot(playlistsQuery, (snap) => {
      const items = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as Playlist));
      setPlaylists(items);
  ...
  useEffect(() => {
    ...
    getDocs(qVideos)
      .then((snap) => {
        const list = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as YouTubeVideo));
        setPlaylistVideos(list);
        setIsLoadingPlaylistVideos(false);

        // Prepopulate first video in custom player if requested
        if (list.length > 0 && !selectedVideo) {
          // If we want to instantly play the series or wait until user clicks
        }
      })
      .catch((err) => {
        console.error("Error retrieving playlist videos:", err);
        setIsLoadingPlaylistVideos(false);
      });
  }, [selectedPlaylist]);
  ...
  const handlePerformSearch = async (e?: React.FormEvent) => {
    ...
    try {
      // Step 1: Query Firestore videos collection. Max 100 entries filtered client-side for zero-quota usage
      const videosRef = collection(db, 'videos');
      
      // Perform paginated chunk of docs (limited to 50 as described)
      let q = query(videosRef, limit(50));
      
      if (filterSubject !== 'All') {
        q = query(videosRef, where('subject', '==', filterSubject), limit(50));
      }

      const snap = await getDocs(q);
      const allVideos = snap.docs.map(doc => ({ id: doc.id, ...doc.data() } as YouTubeVideo));
  ...
  useEffect(() => {
    const qLatest = query(collection(db, 'videos'), orderBy('publishedAt', 'desc'), limit(15));
    getDocs(qLatest)
      .then(snap => {
        const vids = snap.docs.map(doc => doc.data() as YouTubeVideo);
        setVideosState(vids);
      })
              ...
              {isLoadingSearch ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {searchResults.map((vid) => {
                    ...
                    const formattedSub = subCount >= 1000000 
          ...
          >
            <ChannelHeader
              channel={{
                ...
                subscriberCountFormatted: selectedChannel.subscriberCount ? (selectedChannel.subscriberCount >= 1000000 ? (selectedChannel.subscriberCount/1000000).toFixed(1) + 'M' : (selectedChannel.subscriberCount/1000).toFixed(1) + 'K') : '150.3K',
