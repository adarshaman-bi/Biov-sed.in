...
export default function TeacherManager() {
  ...
  useEffect(() => {
    async function loadAllData() {
      ...
      try {
        ...
        const list: TeacherExtended[] = [];
        querySnapshot.forEach((docSnap) => {
          const data = docSnap.data();
          list.push({
            id: docSnap.id,
            name: data.name || '',
            photoUrl: data.photoUrl || data.avatar || 'https://images.unsplash.com/photo-1544717297-fa15c6192437?w=150&auto=format&fit=crop&q=80',
            bio: data.bio || '',
            verifiedBadge: data.verifiedBadge !== undefined ? data.verifiedBadge : (data.isVerified || false),
            claimStatus: data.claimStatus || 'UNCLAIMED',
            isLocked: data.isLocked || false,
            disputeNotes: data.disputeNotes || '',
            commercialDocToken: data.commercialDocToken || '[Verification Identifier Redacted]',
            subjects: data.subjects || (data.subject ? [data.subject] : []),
            exams: data.exams || ['JEE'],
            experienceYears: data.experienceYears !== undefined ? data.experienceYears : 8,
            qualifications: data.qualifications || ['M.Tech', 'B.Tech'],
            currentInstitutes: data.currentInstitutes || (data.instituteId ? [data.instituteId] : []),
            pastInstitutes: data.pastInstitutes || [],
            teachingStyleTags: data.teachingStyleTags || ['Mathematical Rigor', 'Visually Intuitive'],
            aggregatedRating: data.aggregatedRating || data.rating || 4.8,
            ...
            sampleVideoEmbedUrl: data.sampleVideoEmbedUrl || null,
            socialLinks: data.socialLinks || { youtube: data.youtubeChannelId ? `https://youtube.com/channel/${data.youtubeChannelId}` : '' },
            subscriberCount: data.subscriberCount || 0,
            similarTeachersOverrides: data.similarTeachersOverrides || []
          });
        });
        setTeachers(list);
        ...
        instSnapshot.forEach((docSnap) => {
          instList.push({ id: docSnap.id, name: docSnap.data().name || '' });
        });
  ...
  const handleSaveTeacher = async () => {
    ...
    try {
      const docRef = doc(db, 'teachers', finalTeacher.id);
      await setDoc(docRef, {
        ...finalTeacher,
        avatar: finalTeacher.photoUrl, // Backwards-compatible for legacy components
        subject: finalTeacher.subjects[0] || 'Physics', // Backwards-compatible
        isVerified: finalTeacher.verifiedBadge, // Backwards-compatible
        rating: finalTeacher.aggregatedRating, // Backwards-compatible
