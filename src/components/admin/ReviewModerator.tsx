...
export default function ReviewModerator() {
  ...
  const loadAllReviews = async () => {
    ...
    try {
      ...
      const list: ReviewExtended[] = [];
      snap.forEach((docSnap) => {
        const d = docSnap.data();
        const textStr = d.text || d.comment || '';
        list.push({
          id: docSnap.id,
          studentId: d.studentId || d.userId || 'anonymous_student',
          targetType: d.targetType ? d.targetType.toUpperCase() : 'TEACHER',
          targetId: d.targetId || 'unknown_target',
          text: textStr,
          characterCount: textStr.length,
          categoryRatings: d.categoryRatings || [
            { metricName: 'Clarity', score: d.rating || 4 },
            { metricName: 'Punctuality', score: d.rating || 5 }
          ],
          overallCalculatedScore: d.overallCalculatedScore || d.rating || 4.5,
          isAnonymous: d.isAnonymous || false,
          verifiedStudentBadge: d.verifiedStudentBadge !== undefined ? d.verifiedStudentBadge : (d.isVerifiedStudent || false),
          proofDocumentPath: d.proofDocumentPath || null,
          uploadedScreenshots: d.uploadedScreenshots || [],
          upvoteCount: d.upvoteCount || d.trustImpact || 0,
          reportedFlagsCount: d.reportedFlagsCount || (d.flagged ? 1 : 0),
          moderationStatus: d.moderationStatus || 'PENDING',
          adminReply: d.adminReply || null,
          authenticityScore: d.authenticityScore || 85
        });
      });
                  ...
                  <div className="bg-zinc-950 p-4 border border-zinc-900 rounded-xl space-y-2 text-zinc-400 relative">
                    ...
                    <button
                      onClick={async () => {
                        ...
                        await setDoc(doc(db, 'reviews', selectedReview.id), { adminReply: null }, { merge: true });
                        setReviews(prev => prev.map(r => r.id === selectedReview.id ? updated : r));
                        setSelectedReview(updated);
