...
export default function BlockBuilder() {
  ...
  // Load layout configurations from firestore system/global_layout_config
  useEffect(() => {
    async function loadConfig() {
      setLoading(true);
      try {
        const docRef = doc(db, 'system', 'global_layout_config');
        const snap = await getDoc(docRef);
        if (snap.exists()) {
          const fetched = snap.data() as GlobalLayoutConfig;
          ...
          // Put the defaults
          await setDoc(docRef, DEFAULT_LAYOUT_CONFIG);
  ...
  // Save layout configurations to firestore
  const handleSaveToProduction = async () => {
    ...
    try {
      ...
      setSaveStatus('SUCCESS: Global layout config successfully synced with Production Firestore.');
