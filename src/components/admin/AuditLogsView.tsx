...
export default function AuditLogsView() {
  ...
  const loadAuditTrailLogs = async () => {
    setLoading(true);
    try {
      const snap = await getDocs(collection(db, 'system/audit_trail/logs'));
      ...
      console.error('Failed to load system audit trail logs:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSeedMockLog = async () => {
    try {
      ...
      const mockRef = doc(collection(db, 'system/audit_trail/logs'), mockId);
      const mockEntry: SecurityAuditEntry = {
        id: mockId,
        adminUserId: 'mock_test_admin@biovised.in',
        actionType: 'PRICE_OVERRIDE',
        targetCollection: 'BATCHES',
        targetDocumentId: 'batch_neet_fasttrack_2026',
        timestamp: new Date().toISOString(),
        ipAddressSnapshot: '192.168.1.100 (Simulated)',
        deltaChanges: [
          {
      ...
      await setDoc(mockRef, mockEntry);
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-zinc-900 pb-4 mb-4">
        <div>
          ...
          <p className="text-zinc-500 text-xs mt-0.5">
            Cryptographically tracked lifecycle operations logs registered in Firestore collection <span className="font-mono text-zinc-400">/system/audit_trail/logs/*</span>
          </p>
        </div>
