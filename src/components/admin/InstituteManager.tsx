...
export default function InstituteManager() {
  ...
  useEffect(() => {
    async function loadInstitutes() {
      ...
      try {
        ...
        const list: InstituteExtended[] = [];
        snap.forEach((docSnap) => {
          const d = docSnap.data();
          list.push({
            id: docSnap.id,
            name: d.name || '',
            logoUrl: d.logoUrl || d.logo || 'https://images.unsplash.com/photo-1546410531-bb4caa6b424d?w=100',
            bannerUrl: d.bannerUrl || 'https://images.unsplash.com/photo-1541339907198-e08756dedf3f?w=600',
            verifiedInstitute: d.verifiedInstitute !== undefined ? d.verifiedInstitute : (d.isVerified || false),
            branches: d.branches || [],
            instituteType: d.instituteType || 'HYBRID',
            foundedYear: d.foundedYear || 2015,
            affiliatedTeachers: d.affiliatedTeachers || [],
            offeredBatches: d.offeredBatches || [],
            feeStructureDisplay: d.feeStructureDisplay || [],
            scholarshipInfo: d.scholarshipInfo || null,
            overallRating: d.overallRating || d.rating || 4.5,
            ...
            toppersShowcase: d.toppersShowcase || [],
            galleryPhotos: d.galleryPhotos || [],
            contactInfo: d.contactInfo || { phone: '', email: '', website: '' },
            admissionProcessInfo: d.admissionProcessInfo || ''
          });
        });
        setInstitutes(list);
      } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Save to Firestore
      const docRef = doc(db, 'institutes', selected.id);
      await setDoc(docRef, {
        ...selected,
        // sync compatible model names
        logo: selected.logoUrl,
        rating: selected.overallRating,
        isVerified: selected.verifiedInstitute,
