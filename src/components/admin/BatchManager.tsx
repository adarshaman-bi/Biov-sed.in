  ...
  useEffect(() => {
    async function loadAllData() {
      ...
      try {
        ...
        const list: BatchExtended[] = [];
        snap.forEach((docSnap) => {
          const d = docSnap.data();
          list.push({
            id: docSnap.id,
            batchCode: d.batchCode || d.id || '',
            name: d.name || '',
            teacherIds: d.teacherIds || d.teachers || [],
            instituteId: d.instituteId || '',
            subjectIds: d.subjectIds || (d.subject ? [d.subject] : []),
            targetExam: d.targetExam || 'JEE',
            startDate: d.startDate || new Date().toISOString(),
            endDate: d.endDate || '',
            scheduleTimeline: d.scheduleTimeline || [],
            durationTotalMonths: d.durationTotalMonths || 12,
            pricingTiers: d.pricingTiers || [
              { tierName: 'Standard', regularPrice: d.price || 9999, discountPrice: d.price || 9999, features: ['Full Curriculum', 'LMS Access'] }
            ],
            refundPolicyText: d.refundPolicyText || '7-day unconditional money back policy.',
            mode: d.mode || 'LIVE',
            syllabusBreakdown: d.syllabusBreakdown || [],
            enrollmentCount: d.enrollmentCount || 0,
            seatAvailabilityCounter: d.seatAvailabilityCounter || 100,
            demoClassVideoUrl: d.demoClassVideoUrl || null,
            batchRating: d.batchRating || 4.7
          });
        });
        setBatches(list);
